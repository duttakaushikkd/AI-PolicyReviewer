...
const EMBEDDINGS_PATH = process.env.VERCEL
  ? path.join("/tmp", "policy-reviewer-embeddings.json")
...
function requireApiKey() {
  if (!process.env.OPENAI_API_KEY) {
    const error = new Error(
      "OPENAI_API_KEY is not set. Add it to .env.local locally, or as a Vercel environment variable.",
    );
    error.status = 500;
    throw error;
  }
